...
def bentuk_matrix():
    win1 = Tk()
    win1.title("Kalikan Matrix")
    win1.geometry("400x400+120+120")
    win1.configure(bg='#49A')
    ...
    def get_mat():
        ...
        print(temp_mat)
        ##print matriks hasil 
        Label(win1, text="Hasil Perkalian Matrik A dan B :", font=('arial', 10, 'bold'), 
          bg='#0059b3').place(x=20, y=210)
        Label(win1, text=temp_mat, font=('arial', 10, 'bold'), 
    ...
    Label(win1, text="Matrik A :", font=('arial', 10, 'bold'), 
          bg="#0059b3").place(x=20, y=20)

    row_mat1 = int(input_row1.get())
    col_mat1 = int(input_col1.get())
    x2=0
    y2=0
    rows, cols = (row_mat1,col_mat1)
    for i in range(rows):
        ...
        for j in range(cols):
            ...
            entries[i].append(Entry(win1, textvariable=text_var[i][j].get(),width=3))
            entries[i][j].place(x=20 + x2, y=50 + y2)
            x2 += 30
        y2 += 30
        x2 = 0    
    #make matriks B
    Label(win1, text="Matrik B :", font=('arial', 10, 'bold'), 
          bg="#0059b3").place(x=200, y=20)

    text_varB = []
    entriesB = []
    row_mat2 = int(input_row2.get())
    col_mat2 = int(input_col2.get())
    x3 = 0
    y3 = 0
    rows_B, cols_B = (row_mat2,col_mat2)
    for k in range(rows_B):
        ...
        for l in range(cols_B):
            ...
            entriesB[k].append(Entry(win1, textvariable=text_varB[k][l],width=3))
            entriesB[k][l].place(x=200 + x3, y=50 + y3)
            x3 += 30

        y3 += 30
        x3 = 0    
    button= Button(win1,text="Hitung", bg='#0059b3', width=15, command=get_mat)
